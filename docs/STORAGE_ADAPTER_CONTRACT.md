...
2. 遵守 Human Gate；
3. 寫入後 read-back identity/parent/revision；
4. 失敗時回報 `SAVE_FAILED` 或更細 provider code；
5. 無法驗證時回報 `SAVE_UNVERIFIED`。

## Recovery

Recovery 是 adapter-specific：

- Dropbox 可利用 stable ID / revision / restore capability。
